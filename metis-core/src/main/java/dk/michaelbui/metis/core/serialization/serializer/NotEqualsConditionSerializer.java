...
import dk.michaelbui.metis.core.domain.condition.NotEqualsCondition;
import dk.michaelbui.metis.core.serialization.ConditionType;
import dk.michaelbui.metis.core.serialization.Constants;

import java.io.IOException;

public class NotEqualsConditionSerializer extends StdSerializer<NotEqualsCondition> {

    public NotEqualsConditionSerializer() {
        this(null);
    }

    public NotEqualsConditionSerializer(Class<NotEqualsCondition> t) {
        super(t);
    }

    @Override
    public void serialize(NotEqualsCondition value, JsonGenerator gen, SerializerProvider provider) throws IOException {
        gen.writeStartObject();
        gen.writeStringField(Constants.METIS_CONDITION_TYPE, ConditionType.NOT_EQUALS.getValue());
